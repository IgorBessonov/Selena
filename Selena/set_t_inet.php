...
//	$prv_txn = get_new_prv_txn();
	...
	$cr = is_txn_id($txn_id);
//	fdebug("запись с номером $txn_id ".(($cr)?"есть":"отсутствует")."<br>");
...
//	$r_txn = get_prv_txn($txn_id);	//*****
	$sum =  $r_txn["sum"];
	if ($r_txn["result"] != 0 || $r_txn["error"] != 0) {
		fdebug("предыдущий результат - ".$r_txn["result"].", ошибка - ".$r_txn["error"]."<br>");
/*	}
...
//		if ($off) fdebug("Абонент: <b>". (substr(get_000($fp, "on $account"), 6, 7)=="Success"?"успешно":"НЕ")." включен</b><br>");
		...
		if (round(floatval($acc_2)) != round(floatval($acc_1))/*, 2) == round(floatval($sum), 2)*/) {
			$q_upd = "update `t_inet` set result=0, error=0 where txn_id=$txn_id";
			$r_upd =  mysql_query($q_upd) or die(mysql_error());
			//*********************	
			$B_inf = get_inf_acc($account);	// получение данных пользователя
			//*********************	
			fdebug($q_ins_inet = "insert into `act{$Y}` (TabNum,Bill_Dog,Nic,Login,InputDate,Date_start,Date_end,Summa,id_ActionType,Comment) ".
							"values (11,".$B_inf['Bill_Dog'].",'".$B_inf['Nic']."','".$B_inf['Login']."',$now,'".date("Y-m-d")."','".date("Y-m-d")."',".
							"$sum,2,'($tn)".($wNic !=$B_inf['Login']?$wNic." ":"")."$txn_id $acc_1 +')");//echo 	.'</br>'
			$s_ins_inet =  mysql_query($q_ins_inet) or die(mysql_error());
			fdebug($q_ins_inet = "insert into `actions` (TabNum,Bill_Dog,Nic,Login,InputDate,Date_start,Date_end,Summa,id_ActionType,Comment) ".
							"values (11,".$B_inf['Bill_Dog'].",'".$B_inf['Nic']."','".$B_inf['Login']."',$now,'".date("Y-m-d")."','".date("Y-m-d")."',".
							"$sum,2,'($tn)".($wNic !=$B_inf['Login']?$wNic." ":"")."$txn_id $acc_1 +')");//echo 	.'</br>'
			$s_ins_inet =  mysql_query($q_ins_inet) or die(mysql_error());
			//*********************	
			...
			$resp = $sum==0?0:90;
			$q_upd = "update `t_inet` set result=$resp, error=5 where txn_id=$txn_id";
			$r_upd =  mysql_query($q_upd) or die(mysql_error());
		}
	}
	fdebug("---------------------<br>");
...
function Go_Out($txn_id, $resp) {
	fdebug("выходим!");
	echo "<a href='javascript:{ch_param(\"sh_form\",\"menu=pay&tn=".$GLOBALS['tn']./*'&tp='.$tp.*/"&Bill_Dog=".$GLOBALS['B_inf']['Bill_Dog']."\",\"Mform\"); s_Bill_Dog();}'>".$GLOBALS['B_inf']['Bill_Dog']."</a>";
