...
//			.' '.($tp<3?$row_customer['Fam']:html_entity_decode(substr($row_customer['Fam'],1,1).'.', ENT_NOQUOTES,"UTF-8"));
...
if ($GLOBALS['totalRows_customer']>0) {
	...
	$Fam = $tp<3?$row_customer['Fam']:html_entity_decode(chr(ord($a_cus[$Bill_Dog1]['Fam'])).ord($a_cus[$Bill_Dog1]['Fam']).'.');//($tp<3?$row_customer['Fam']:(chr(ord($row_customer['Fam'])).'.'))
...
<? if(!$m_pay) {?>
    	друг: <input name="Bill_frend" size="4" value="<? echo isset($a_cus[$Bill_Dog]['Bill_frend'])?$a_cus[$Bill_Dog]['Bill_frend']:"" ?>" <? echo $GLOBALS['tp']<3?'onchange="adjastNet()"':''?>/>
<? }?>
	</div></td>
  </tr>
</table>
