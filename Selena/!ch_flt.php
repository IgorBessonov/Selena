...
/* $Bill_Dog = 0;
 ...
 if (!$not_new_adr) { //$Nic!=""
	...
	$q_customer = "SELECT * FROM v_customer where id_Podjezd=".$id_Podjezd." and flat =".$fl." order by inet";
//	$q_customer = "SELECT * FROM customers_xls where (Nic not in (select Nic from customers)) and id_Podjezd=".$id_Podjezd." and flat =".$fl; 
...
//  	echo '<input name="h_" type="hidden" value="'.$.'" />';
	...
	if ($totalRows_customer > 0) {?>
		<button name="B_reload" type=button onClick="clr_adress(); chk_adress();"><img src="reload.png" align=middle alt="Обнови"></button>
<?	}
...
//			echo //' <input name="B_edt_cust" type="button" onclick='.$s_onchange.' value="Показать" />';phone_Home=form.phone_Home.value; 
		?>	&nbsp;
			...
			<button name="B_del_cust" type=button 
            	onClick="javascript:B_D=f_Bill_Dog();tn=<? echo $tn?>;if(confirm(&quot;Вы согласны удалить договор№&quot;+B_D+&quot; в архив? &quot;)){ch_param('do_del_cust','Bill_Dog='+B_D+'&tn='+tn,'tab_Cust');}"><img src="ico_delete.gif" align=middle alt="Удалить"></button>
		  </td><td>
			<button name="B_print" type=button onClick="javascript:dogovor();"><img src="printer.gif" width="16" alt="Печать"></button>
			<button name="B_pays" type=button onClick="javascript:document.forms.ulaForm.Menu_Item.value='pay';ch('srch','menu=pay&tp=<? echo $tp.($new_Cod?'&Bill_Dog='.$Bill_Dog1:'&Cod_flat='.$Cod_flat)?>',0,'tab_Cust');">Финан</button>
<?		} elseif ($_REQUEST ["menu"] == "pay") {	?>
		<div id="d_b_pay">
			<button name="B_pays" type=button onClick="document.forms.ulaForm.Menu_Item.value='recon';ch('srch','menu=recon&tp=<? echo $tp?>&Bill_Dog=<? echo $Bill_Dog1?>',0,'tab_Cust');">Сеть</button>
	<? if($tp==1 || $tn==6 || $tn==8) { // (!$inet1 && ($tn==2 || $tn==6)) ?>
    <button name="B_dhd" type=button onClick="toggle('dhd')">▼</button>
<div id="dhd" style="display:none">
<!--    </br>--><table><tr><td><? echo $Bill_Dog1?>:</td>
	<?	if($inet1) {?>
        ...
        <? } else { ?>
        	<?php /*?><td><div id="rest"><button name="B_rest" type=button onClick="javascript:ch_param('rest','tn=<? echo $tn?>&Bill_Dog=<? echo $Bill_Dog1?>','rest');">Вос</button></div></td><?php */?>
        	<td><div id="otkat" style="background-color:#9FF">
            	&nbsp;<!--<input name="n_otk" value="0" size="2" onchange="otkat_chk();"/>-->
		...
		if ($GLOBALS['a_cus'][$Bill_Dog1]['state']==1 && (strtotime ($GLOBALS['a_cus'][$Bill_Dog1]['Date_end_st']) < $d_test || strtotime ($GLOBALS['a_cus'][$Bill_Dog1]['Date_pay']) < $d_test) ) { 
			...
			if($D_e=="" || $D_p=="") {
?>        		<td><div id="d_cor" style="background-color:#F99"><button type=button onClick="javascript:ch_param('d_cor','B=<? echo $Bill_Dog1?>&d=<? echo $D_e==""?"p":"e"?>','d_cor');setTimeout('f.sBill_Dog.onchange()', 500);">Дата</button></div></td>
		<?	}
		}?>
    </tr></table>
</div>
	...
	<? } elseif (isset($_REQUEST ["Nic"]) || $_REQUEST ["menu"] == "show_err") {//<button id="sel_Bill" type="button">уст</button>
       ...
       <!--     <button name='B_chng' type='button' onClick='alert(document.forms.ulaForm.hNic.value)' >       -->
				<label><input name="sel_Dog" type="radio"
                	value="<? echo $cust_row['Bill_Dog']?>" <? if(!$cust_row['inet']){?>disabled="disabled"<? }?>
...
<?			}	?>
			</td><td><div id="b_chng"><? if ($noNic) {?><? } else echo $_REQUEST ["Nic"]?> -> Абон.ник<br><? echo $cust_row["Nic"]?> -> Инет.логин</div>
<?		}	?>
		</td></tr><FONT size=-1><table><tr>
...
//	if {
		$m_dolg = ($state==2 && $Date_end_st=='')?(strtotime($Date_start_st) - strtotime($Date_pay))/60/60/24:0;
//	}
...
<b><font style="font-size:14px">&nbsp;
	<label id="lpay"><input name="B_sel" type="radio" onclick="f_sel(this.value);" value="pay" /> Платёж </label>&nbsp;
	<label id="lall" style="background-color:#CCFF99"><input name="B_sel" type="radio" onclick="f_sel(this.value);" value="all" checked/> Платёж поровну </label>&nbsp;
	...
	<label id="lnoti"><input name="B_sel" type="radio" onclick="f_sel(this.value);" value="noti" /> Операции и заявки </label>
</font></b>
  ...
  <tr>
    ...
    <div id="pay" <? echo 'style="display:none"' ?>>
		<table width="800" border=0 cellpadding="2" cellspacing="2" bgcolor="#66FF99">
		  ...
		  <tr id="r_dolg" <?	$c_ar=$a_cus[$Bill_Dog];	$is_dolg=($state==2 && $Date_end_st=='' && $c_ar['auto']==0)||($c_ar['auto']==1 && $state==0); if(!$is_dolg){ ?>style="display:none"<? } ?>>
<?				$m_ab = isset($GLOBALS['ab_numbs'])?($id_tarifab==6&&$c_ar['Comment']!=''?$c_ar['Comment']:round($c_ar['ab_sum']/2*(1+1/($GLOBALS['ab_numbs']+1/*>0?$GLOBALS['ab_numbs']:1*/)))):'';
            ...
            <td><b><? if($m_dolg>0) {?>Оплата долга <? } echo ($is_dolg?"+ 100руб.":"авто")."переподкл"?> </td>
		  </tr>
		  <tr>
		  ...
		  <tr>
			<td>Интернет <button name="refr_3w" type=button onClick="f = document.forms.ulaForm; Bill_Dog = f_Bill_Dog();
		ch_param('refr_3w','tn=<? echo $tn?>&account='+val_nm(Bill_Dog, 'account'+(1*f.Login.selectedIndex+1)), 'res_pay');"><img src="reload.png" align=middle alt="Обнови"></button>: </td>
		  ...
		  <tr>
			...
			<td align="right"><input name="total_pay" id="all_cost" size="3"  align="right" value="<? if($is_dolg) {echo 100+$s_dolg;}//readonly="true"?>" onchange="<? if($n_cod) { ?>adj_pay('tot')<? } else {?>alert('не присвоен код адреса')<? }?>"/></td><td > руб.</td>
			<td colspan="5">
	...
	<div id="all"<? //if ($dolg) { echo ' style="display:none"'; }?>>
		<table width="640" border=0 cellspacing="3" bgcolor="#CCFF99">
		  <tr id="a_dolg"<? if (!$dolg) { echo ' style="display:none"'; }?>>
                  ...
                  <tr>
                    ...
                    <td width="19" >
		              <input name="opl_" type="text" size="3" align="right" onchange="
					  	<? if($n_cod) {?>
                            f=document.ulaForm; f.ab_.value=f.opl_mon.value*f.h_ab_numbs.value*this.value; adj_pay()
		...
		<table width="640" border=0 cellspacing="2" bgcolor="#CCFF99">
		  <tr bgcolor="#CCCC99">
			<td align="center" width="73"><b>Договор №</b></td>
			<td align="center" width="78"><b>оплачено по</b></td>
			<td align="center" width="94"><b>сумма</b></td>
			<td align="center" width="96"><b>платёж по</b></td>
			<td width="153" align="center"><b>акция</b></td>
			<td align="center"><b></b></td>
		  </tr>
<?		$i = 0;
//print_r($a_cus);
		foreach ($a_cus as $cust_row)  {
			if($cust_row['inet']==0 && !$cust_row['dolg']){ 
				$i++	// id="ND_<? echo $i? >"  ?>
			<tr>
				<td><label>&nbsp;<input name="ND_<? echo $i?>" type="checkbox" value="<? echo $cust_row['Bill_Dog']?>"
							onchange="" checked="checked" /><? echo $cust_row['Bill_Dog']?></label></td>
				<td align="center" colspan="1"><? echo $cust_row['Date_pay']?>
                	<input name="D_st_<? echo $i ?>" value="<? echo $cust_row['Date_start_st']?>" type="hidden"/></td>
...
<?			}
			elseif (gettype($cust_row)!='string') { ?>
			<tr>
				<td align="center"><? echo $cust_row['Bill_Dog']?></td>
				<td align="center"><? echo $cust_row['Date_pay']?></td>
                <td colspan="4"><? echo ($cust_row['dolg']?'<b>Нет заявки на откл. Сделайте переоформление!</b>':'')/*.($cust_row['state']==2?' Отключен':'Подключен').' с '.$cust_row['Date_start_st'].
					($cust_row['state']==2 && $cust_row['Date_end_st']==''?' за долг':' по '.$cust_row['Date_end_st'])*/ ?> </td>
			</tr>
...
<!--            <tr>
				<td colspan="7">-->
					<table border=0 width="640" bgcolor="#CCFF99">
					  <tr bgcolor="#CCCC99">
						<td width="23"></td>
						<td align="left"><div class="quote" id="t_pay"></div></td>
...
<!--				</td>
    ...
    <!-------------------------------------------------------------------------------------->
    <div id="hist_pay" style="background-color:#E5E5E5<? echo $dolg?'':'; display:none' ?>"></div>
    <!----------------------------------------------------------------->
    <div id="hist_cod" <? echo 'style="display:none"'?>></div>
    <!----------------------------------------------------------------->
    <div id="noti" <? echo 'style="display:none"'?>></div>
</div>
<?
}
//============================================================================
...
<table id="net_tab" width="800" border=0 <?php echo (($m_pay && $a_cus[$Bill_Dog]['inet']==1)?'style="display:none"':'');?>>
  <tr>
	...
	<?php if (!$m_pay) {?>&nbsp;Договор<? }?>
      	<input name="Bill_Dog" id="Bill_Dog" value="<? echo $Bill_Dog; ?>" type="text" <?php echo ($m_pay?'style="display:none"':'');?> size="4" onchange='adj_Bill_Dog(this);' <? if($Bill_Dog=="новый") {?> disabled="disabled"<? }?>/>
		<?php if (!$m_pay) {?>MAC<? 
        ...
        <input name="mac" id="mac" value="<? echo $mac; ?>" type="text" <? if($m_pay){?>style="display:none"<? }?> size="15" onchange='adj_mac();' maxlength="17" onkeyup="v_MAC(this.value);//mac_up(this.value);" />
		<?php if (!$m_pay) {?>&nbsp;ник<? }?>
...
/*echo $conn," ",*/	$s_qer = "SELECT * FROM `v_tarifab` WHERE `con_typ`=".$c_typ.(($c_typ==4)?" or (`id_tar_con`=1 and `con_sum`<500)":"")." order by `id_tar_con`";
			...
			$nm_tarif = "";	?>
		<select name="id_tar_con" class='font8pt' id="id_tar_con" onchange='adj_con_tar(this);' <?php if($m_pay) {//disabled?><? }?> <?php if($m_pay){?>style="display:none"<? }?>>	
...
<?		foreach ($tarifs as $t_row) {		//	print_r($t_row);
    ...
    <td align="left" <?php echo ($m_pay?'style="display:none"':'');//<div id="conn_pay" ></div>?>>
    	<input name="conn_pay" type="text" size="3" onchange="adj_CPay()" /> руб.
...
<? if(!$m_pay) {?>
    	друг: <input name="Bill_frend" size="4" value="<? echo isset($a_cus[$Bill_Dog]['Bill_frend'])?$a_cus[$Bill_Dog]['Bill_frend']:"" ?>" onchange="adjastNet()"/>
<? }?>
	</div></td>
  </tr>
</table>
