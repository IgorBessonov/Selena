...
`state`, `Date_start_st` , `Date_end_st` , `Date_pay` 
FROM `v_customer` 
WHERE 
		id_tarifab IN ( $tr ) 
	and isnull(inet)
		order by name_street,Num_build,Korpus,Podjezd,flat";
$result = mysql_query($s_quer);		// Выполняем запрос 1, 5,	AND `state` =1	AND `Bill_Dog` NOT IN ( SELECT `Bill_Dog` FROM v_is_noti_off ) 
if (!isset($result)) {
	echo "не найдены";
	return;
}
...
//			echo '<td>'.' '.'</td>';
		...
		<? } ?>
<? if($tp) { ?>		<td bgcolor="<?php echo $bgcolor; ?>"> <?php echo $row["Date_pay"];	?> </td><? } ?>
