...
{
	echo " Счёт $Bill_Dog";
	$do = isset($_REQUEST ["do"])?$_REQUEST ["do"]:'2010-10-19';
...
//	do {
	...
	$q_o = "delete from actions where Bill_Dog=$Bill_Dog and InputDate>='$do';";
	$res = mysql_query($q_o) or die(mysql_error());
	$q_o = "delete from act2012 where Bill_Dog=$Bill_Dog and InputDate>='$do';";
	$res = mysql_query($q_o) or die(mysql_error());
	$q_o = "delete from act2011 where Bill_Dog=$Bill_Dog and InputDate>='$do';";
	$res = mysql_query($q_o) or die(mysql_error());
	$q_o = "delete from act2010 where Bill_Dog=$Bill_Dog and InputDate>='$do';";
	$res = mysql_query($q_o) or die(mysql_error());
	echo "операции";//, mysql_fetch_assoc($res)
	$res = mysql_query("delete from notify_repair where Bill_Dog=".$Bill_Dog) or die(mysql_error());
	echo ", заявки";//, mysql_fetch_assoc($res)

	$res = mysql_query("delete from logins where Bill_Dog=".$Bill_Dog) or die(mysql_error());
	echo ", логины удалены"."</br>";//, mysql_fetch_assoc($res)
...
//+++++++++++++++++++++++++++++++
		...
		$q_ins_noti = "insert into `notify_repair` (id_p,fl,TabNum,Bill_Dog,Nic,Date_in,Date_Plan,conn,Notify) ".
						"values (".$row['id_Podjezd'].",".$row['flat'].",2,".$rowNic['Bill_Dog'].",'".$row["Nic"]."','".
