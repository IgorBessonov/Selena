...
$tp = !(isset($_REQUEST ["tp"])?($_REQUEST ["tp"]==4):1);
 ...
 $q_dolgn = "SELECT * from v_customer where ( ((`Date_pay` is null) or (`Date_pay`<'".date('Y-m-d')."')) $tn $k and ".
				"(inet is null) and (`state` is not null or `state` <>4) and (`id_tarifab` <> 3) )".
...
<? if($tp) { ?>		<td bgcolor="<?php echo $bgcolor; ?>">Оплата по</td><? } ?>
		<td bgcolor="<?php echo $bgcolor; ?>">Срок долга</td>
	</tr>
...
<? if($tp) { ?>		<td bgcolor="<?php echo $bgcolor; ?>"> <?php echo $row["Date_end_st"];	?> </td><? } ?>
		<td bgcolor="<?php echo $bgcolor; ?>"> <?php echo round((strtotime("now") - strtotime($row["Date_pay"]))/60/60/24, 0), " дн.";	?> </td>
		</tr>		<?php 
//		$SDolg += $row["Saldo"];
    }
?>
...
<input name="Menu_Item" type="hidden" value="dolgn" />
