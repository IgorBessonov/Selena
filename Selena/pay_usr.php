...
//$res = f1("");
fill_actions('v_pays_frm');
fill_actions('v_pays_nic');
AnalizCust();
//StateAnaliz();
/*
insert into logins (
...
function AnalizCust()
{
	...
	//+++++++++++++++++++++++++++++++
			$q_ins_noti = "insert into `notify_repair` (id_p,fl,TabNum,Bill_Dog,Nic,Date_in,Date_Plan,conn,Notify) ".
							"values (".$row['id_Podjezd'].",".$row['flat'].",2,".$rowNic['Bill_Dog'].",'".$row["Nic"]."','".
									date("Y-m-d H:i:s")."','".$rowNic["Date_end"]."',-1,'откл.(долг)')";
	...
	echo "Пометка своих</br>";
	$q_upd = mysql_query("update customers set id_tarifab=3 where Nic in (select Nic from v_svoi)") or die(mysql_error());
	echo "Удаление заявок монтажникам на удаление своих</br>";
	$q_upd = mysql_query("delete from notify_repair where conn=-1 and Notify='откл.(долг)' and Nic in (select Nic from v_svoi)") or die(mysql_error());
