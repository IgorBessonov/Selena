...
//		if ($off) fdebug("Абонент: <b>". (substr(get_000($fp, "on $account"), 6, 7)=="Success"?"успешно":"НЕ")." включен</b><br>");
		...
		if (round(floatval($acc_2)) != round(floatval($acc_1))/*, 2) == round(floatval($sum), 2)*/) {
			...
			if ($B_inf['Bill_Dog']>0) {
				...
				if ( get_ab_sum($account)>0 && $D_pay < mktime()) {
...
/*		ОТКЛЮЧИТЬ		*******/
			...
			$q_ins_inet = "insert into `act{$Y}` (TabNum,Bill_Dog,Nic,Login,InputDate,Date_start,Date_end,Summa,id_ActionType,Comment) ".
				"values (11,".($B_inf['Bill_Dog']>0?($B_inf['Bill_Dog'].",'".$B_inf['Nic']."','".$B_inf['Login']):"0,'','".$wNic).
							...
							"$sum,2,'".($wNic !=$B_inf['Login']?$wNic." ":"").($B_inf['Bill_Dog']>0?"":"($account) ")."$txn_id $acc_1 +$s_st')";//echo 	,'</br>'
...
function get_ab_sum($acc)
{
	if(($bd = get_Bill_acc($acc))==0) return 0;
	$q = "SELECT ab_sum FROM v_customer where Bill_Dog = $bd" ;
  	$rq = mysql_query($q) or die(mysql_error());
	$r_ = mysql_fetch_assoc($rq);
	return $r_["ab_sum"];
}
