...
//		for (var key in asel) {     var val = asel [key];     alert (key+' = '+val); } 
		var asel = [{'nm' : 'pay', 'rgb' : '#66FF99', 	'prg' : '', 'bd':''},
					{'nm' : 'all', 'rgb' : '#CCFF99', 	'prg' : '', 'bd':''},
					{'nm' : 'frz', 'rgb' : '#99FFFF', 	'prg' : '', 'bd':''},
					{'nm' : 'hist_pay', 'rgb' : '#E5E5E5', 	'prg' : 'hist_pay', 'bd':'+'},
					{'nm' : 'hist_cod', 'rgb' : '#CCCCCC', 	'prg' : 'hist_cod', 'bd':'+'},
					{'nm' : 'hist_not', 'rgb' : '#0CF', 	'prg' : 'hist_not', 'bd':'+'}];
		f = document.forms.ulaForm;
		f.sel.value = dv;
		...
		el = asel.filter( function(item){return item.nm==dv;} )[0];
	...
	return;
		document.getElementById('lpay').style.backgroundColor='#FFF';
		document.getElementById('lall').style='background-color:#FFF';
		document.getElementById('lfrz').style='background-color:#FFF';
		document.getElementById('lhist_pay').style='background-color:#FFF';
		document.getElementById('lhist_cod').style='background-color:#FFF';
		document.getElementById('lhist_not').style='background-color:#FFF';
		document.getElementById('pay').style.display='none';
		document.getElementById('all').style.display='none';
		document.getElementById('frz').style.display='none';
		document.getElementById('hist_pay').style.display='none';
		document.getElementById('hist_cod').style.display='none';
		document.getElementById('hist_not').style.display='none';	
		document.getElementById(dv).style.display='';
//		toggle(dv);
		if (dv=='pay') { rgb = '#66FF99' }
		if (dv=='all') { rgb = '#CCFF99' }
		if (dv=='frz') { rgb = '#99FFFF' }
		if (dv=='hist_pay') { rgb = '#E5E5E5';
			if (document.getElementById("hist_pay").innerHTML == '') { 
				ab_s = document.getElementById('h_'+Bill_Dog+'_ab_sum').value;
				ab_n = f.h_ab_numbs.value;
				ch_param("sh_pays","tp="+f.tp.value+"&tn="+f.TabNum.value+"&Bill_Dog="+Bill_Dog+"&ab_s="+ab_s+"&ab_n="+ab_n,dv);
			}
		}
		if (dv=='hist_cod') { rgb = '#CCCCCC'; 
			if(document.getElementById("hist_cod").innerHTML == '') {
				ch_param("hist_cod", "Bill_Dog="+Bill_Dog, dv);
			}
		}
		if (dv=='hist_not') { rgb = '#0CF'; 
			if((Bill_Dog!=f.selBill.value) && (document.getElementById(dv+f.selBill.value))) {
					hid(dv+f.selBill.value);
			}
			if(document.getElementById(dv_BD)) {
				...
				document.getElementById(dv).innerHTML += '<div id="'+dv_BD+'"></div>';
				ch_param("sh_noti", "tp="+f.tp.value+"&tn="+f.TabNum.value+"&Bill_Dog="+Bill_Dog, dv_BD);
			}
 		}
		document.getElementById('l'+dv).style='background-color:'+rgb;
		f.selBill.value = Bill_Dog;
}
//---------------------------------------------------------------------------------
	function chk_adress() {
...
/*		h_ab_numbs = document.getElementById('h_ab_numbs')?document.getElementById('h_ab_numbs').value:1;
		...
		document.getElementById("con_s").innerHTML = t>0?
			'Абон.тариф <b><font'+(t==3?' size="+1" style="background-color:#FF0000" color="#FFFF00">'+nm+'</font></b>':'>'+nm+'</font></b>&nbsp;'+f.opl_mon.value+' руб./мес.'):'';
...
//				alert(f.selBill.value+' '+Bill_Dog);
		if (document.getElementById('fin').style.display ==''){//f.Menu_Item.value == "pay") {//'<font style="border:solid" color='+ar_s[i_st]+
...
//			document.getElementById("a_dolg").style.display = dolg?"display":"none";
	...
	//			document.getElementById("c_dolg").innerHTML = 'Подключить '+m_dolg+' дней';
			...
			document.getElementById("state").innerHTML += (v_Date_start_st!=''?' с <b>'+(y1==y2?d2strS2(d_Date_start_st):
						d2str2(d_Date_start_st))+'</b>':'')+(v_Date_end_st!=''?' по <b>' + d2str2(d_Date_end_st)+'</b>':
				is_off?' за долг':'')+
				'<input name="Date_start_st" type="hidden" value="'+v_Date_start_st+'" />'+
				'<input name="Date_end_st" type="hidden" value="'+v_Date_end_st+'" />&nbsp;';//</font>
...
//		$d_test = strtotime ("2000-01-01");
					...
					document.getElementById("d_b_pay").innerHTML += '<button name="B_dhd" type=button'+
						' onClick="toggle(\'dhd\')">▼</button>'+
						'<div id="dhd" style="display:none"><table><tr><td>'+Bill_Dog+':</td>'+
						'<td></button><div id="otkat"><button name="B_otkat" type=button onClick="javascript:'+
						'ch_param(\'otkat\',\'Bill_Dog='+Bill_Dog+'\',\'otkat\');">Откт</div></td>'+
						'<td><div id="toinet"><button name="B_2inet" type=button onClick="javascript:'+
						'f=document.forms.ulaForm;f.sBill_Dog.value='+Bill_Dog+';ch_param(\'toinet\',\'B='+Bill_Dog+
						'\',\'toinet\');setTimeout(\'f.sBill_Dog.onchange()\', 500)">3w</button></div></td>'+
						((n_st==1&&(d_Date_end_st<d_test||d_Date_pay<d_test||v_Date_end_st=="0000-00-00"||v_Date_pay=="0000-00-00") )&& 
						(D_e=="" || D_p=="")?'<td><div id="d_cor" style="background-color:#F99"><button type=button onClick="javascript:ch_param(\'d_cor\',\'B='+
																	Bill_Dog+'&d='+(D_e==""?"p":"e")+'\',\'d_cor\');setTimeout(\'f.sBill_Dog.onchange()\', 500);">Дата</button></div></td>':'')+
						'</tr></table></div>';
				}
//				alert(f.selBill.value+' '+Bill_Dog);
...
//					document.getElementById("hist_cod").innerHTML = '';
//					document.getElementById("hist_not").innerHTML = '';
					f_sel(f.sel.value);
				}
				...
				if (f.tp.value == 1 || f.TabNum.value == 6 || f.TabNum.value == 8) {
					document.getElementById("d_b_pay").innerHTML += '<button name="B_dhd" type=button'+
						...
						'<div id="dhd" style="display:none"><table><tr><td>для '+Bill_Dog+'</td>'+
					'<td><div id="toab"><button name="B_2ab" type=button onClick="javascript:f=document.forms.ulaForm;'+
...
//			f.conn[val_nm(Bill_Dog, "con_typ")-1].selected = true;
			...
			document.getElementById("state").innerHTML += 
				...
				' по <input name="Date_end_st" value="'+v_Date_end_st+'" size="8" type="date"/>';
...
//			s_LonChange = 'ch_param(&quot;is_Login_Free&quot;,&quot;Login=&quot;+this.value+&quot;&prm='+prm+'&quot;,&quot;addLogin&quot;);';
		document.getElementById("Login").innerHTML = 'логин '+
			'<input type="button" id="nic2login" value="как ник" '+
				'onclick= "f=document.forms.ulaForm;f.Login.value=f.Nic.value;'+
				'ch_param(&quot;is_Login_Free&quot;,&quot;Login=&quot;+f.Nic.value+&quot;&prm='+prm+'&quot;, &quot;addLogin&quot;);" />'+
			'<input name="Login" type="text" value="" size="12" '+
				'onChange="ch_param(&quot;is_Login_Free&quot;,&quot;Login=&quot;+this.value+&quot;&prm='+prm+'&quot;, &quot;addLogin&quot;);" />'/*+
...
//		document.getElementById("id_tarifab").readOnly=true;
		Bill_Dog = (!document.getElementById("tabl_cust"))?0:f.tabl_cust.options[f.tabl_cust.selectedIndex].value;
		if (document.getElementById("tabl_cust") && (f.Nic.value == val_nm(Bill_Dog, "Nic")) &&	(f.Menu_Item.value == "recon")) {
			document.getElementById("Nic").readOnly=true;
			document.getElementById("Bill_Dog").readOnly=true;
/*			if(f.Date_start_st.value=='') { alert('f.Date_start_st.value'+f.Date_start_st.value) }
			if(f.Date_end_st.value=='') { alert('f.Date_end_st.value'+f.Date_end_st.value) }
			if(f.Date_pay.value=='') { alert('f.Date_pay.value'+f.Date_pay.value) }	*/
			document.getElementById("B_Edit").innerHTML = (f.Date_start_st.value=='' || f.Date_end_st.value=='' || f.Date_pay.value=='')?
				...
				'<input type="button" name="Submit_cor" id="Submit_cor" value="Изменить" onClick="cor_cust();document.getElementById(\'B_Edit\').innerHTML = \'\'" />';
		}
	}
//---------------------------------------------------------------------------------
...
/*		if (!f.id_tar_con) {
		...
		if (f.conn[2].selected) { f.h_ab_numbs.value = 1*f.h_ab_numbs.value + 1; }
		m_opl=opl_p>0?Math.round(f.abon_pay.value/2/opl_p*(1+1/(f.h_ab_numbs?(f.h_ab_numbs.value>0?f.h_ab_numbs.value:1):1))):0;
		document.getElementById("con_s").innerHTML = t>0?'Абон.тариф <b>'+document.getElementById("h_nm_"+t).value+'</b> '+m_opl+' руб./мес.':'';	//"h_id_"+t
...
//		if (f.Date_start_st.value=='') { f.Date_start_st.value=TODAY2; }
		if (f.Date_start_st.value=='') { f.Date_start_st.value=d2str(date_add(new Date(), "day", 1)); }
...
//		f.Date_end_st.value = opl_p==''?f.Date_start_st.value:Date_Add(f.Date_start_st.value,opl_p,0); //n_Y+"-"+n_M+"-"+s_D;
		...
		document.getElementById("state").innerHTML += 
			...
			' по <input name="Date_end_st" value="'+v_D_end+'" size="8" type="date"/>';
		adj_CPay();
    }
//---------------------------------------------------------------------------------
...
function adj_mac() {
	...
	if(macPattern.test(document.forms.ulaForm.mac.value)) adjastNet();
		else document.getElementById("B_Edit").innerHTML = '';
}
//---------------------------------------------------------------------------------
function cor_mac(bd) {
	var macPattern = /^([0-9A-F]{1}[0-9A-F]{1})\-([0-9A-F]{1}[0-9A-F]{1})\-([0-9A-F]{1}[0-9A-F]{1})\-([0-9A-F]{1}[0-9A-F]{1})\-([0-9A-F]{1}[0-9A-F]{1})\-([0-9A-F]{1}[0-9A-F]{1})$/;
...
//	var dmac= "d"+document.getElementById("net")?"":bd;
	if(macPattern.test(m=document.getElementById("mac"+b).value)) {
		if (confirm("Изменить?")) ch_param("do_cor_mac", "bd="+bd+"&m="+get_mac(bd)+"&TabNum="+document.getElementById("TabNum").value, "d"+bd);
...
//	mac += document.getElementById("net")?"":bd;
	...
	if ( !smb.match(macPatt) ) {
		...
		MAC_val = MAC_val.substring(0,MAC_val.length-2);
	}
...
//		else return;
	if(macPattern.test(MAC_val)) if(MAC_val.length==17 && ipArray.length != 7) errorString = /*ipArray[1]+.length+" "+imac+*/"Ошибка в МАС-адресе!";
...
//			setTimeout('document.forms.ulaForm.chk_Login.Click()', 300);
			setTimeout('ch_param("is_Login_Free","Login='+nic.value+'&prm='+prm+'","addLogin");', 300);
		}
		document.getElementById("B_Sub").innerHTML ="";
//		f.Nic.value = val_nm(Bill_Dog, "Nic");
//		document.getElementById("Nic").readOnly=true;
...
//---------------------------------------------------------------------------------
	...
	var f = document.forms.ulaForm;
		var t = f.id_tar_con.options[f.id_tar_con.selectedIndex].value; // № тарифа подключения
		var opl_p = document.getElementById("h_op_"+t).value;//
		f.total_pay.value = 1*f.conn_pay.value + 1*opl_p*f.opl_mon/*abon_pay*/.value + 1*f.inet_pay.value;
	}
//---------------------------------------------------------------------------------
...
//---------------------------------------------------------------------------------
	function d2str(d1) {
		return new String(d1.getFullYear() +"-"+(d1.getMonth()<9?"0":"")+ (1*d1.getMonth()+1) + "-" +(d1.getDate()<9?"0":"")+ d1.getDate());
	}
//---------------------------------------------------------------------------------
	function adj_pay(t_) {
		var f = document.forms.ulaForm;
		var f_all = document.getElementById('all').style.display!='none';
		if (f.h_new_Cod.value == 1) { alert('не присвоен код адреса');	return;	}
...
// *	d2[i] = new Date(val_nm(Bill_Dog[i], "Date_start_st"));	// Date(f.Date_start_st.value);
	...
	//		document.getElementById(""+i).
			...
			if (v_st == '') { alert('Это ИНТЕРНЕТ учётка! Выполните переоформление-подключение к сети'); return; }
...
//			opl_mon = val_nm(Bill_Dog, "ab_sum")
			...
			if (dolg) {
				comm = val_nm(Bill_Dog, "Comment");
				ab_numbs = (f.h_ab_numbs?f.h_ab_numbs.value:1)*1 + 1*1;
				opl_mon = f.h_ab_numbs?(t==6 && comm!=''?comm*1:Math.round(val_nm(Bill_Dog, "ab_sum")/2*(1+1/ab_numbs))):'';
			} else {
	...
	//		alert(f.total_pay.value);//?0:'f.s_dolg.value'
			...
			var d2_ = dolg?d1_:(new Date(f.Date_pay.value));	//date_add(d1_, "day", 1)	d2_ = Date_pay оплачено по ...
...
//				s_act = md>0?'Необходимо оплатить долг за '+md+'мес.':'';
	...
	/*			if(){// отключен за долг
				...
				f.total_pay.value = ''; f.opl_per.value = ''; f.abon_p.value = ''; 
				alert ('Необходимо выполнить переподключение или переоформление.'); return;// состояние ни 1 и ни 2
			}
	//		alert(d2str2(d1)+" ? "+d2str2(d2)+" = "+action);
	...
	//		}
			...
			} else if (!f.Login) { alert ("Отсутствует логин интернета"); }
		}
	}
//---------------------------------------------------------------------------------
	function ins_all() {
	var f, con;
		...
		if (f.Login) { vLogin = f.Login.value; }
...
//		setTimeout('f.B_chk_adress1.click();', 300);
		document.getElementById("hist_pay").innerHTML = '';
...
//alert('menu='+f.Menu_Item.value+'&tp='+f.tp.value+'&'+prm+'='+prm_v);
...
//---------------------------------------------------------------------------------
	...
	var f, con, vLogin;
		...
		if (f.Menu_Item.value == "recon") {
			...
			if (!f.Login) { alert("Не установлен Login интернет!"); return 0 }
...
//	корректировка абонента
			...
			ch_param('do_cor_cust', s_cor_str, 'B_Edit');	//B_Sub
//			ajax.open(smetod, "do_cor_cust"+cor_str(), true);
		} 
		if (f.Menu_Item.value == "noti") {
// оформление заявки на ремонт.php?
			Bill_Dog = (!document.getElementById("tabl_cust"))?0:f.tabl_cust.options[f.tabl_cust.selectedIndex].value;
			c_str = "id_p="+f.h_id_Podjezd.value+"&fl="+f.h_fl.value+"&Notify="+f.noti.value+"&Bill_Dog="+Bill_Dog+
				"&Date_Plan="+f.Date_Plan.value+"&phone_Dop="+f.phone_Dop.value+
				"&TabNum="+f.TabNum.value+"&Cod_flat="+f.h_Cod_flat.value;//+"&Date_in="+f.n_date.value
...
//		alert("Старая дата "+f.Date_end_st.value+", новая дата "+n_Date_end_st+" = "+);
		s_param = "TabNum="+f.TabNum.value+"&Bill_Dog="+Bill_Dog+"&Date_start_fr="+f.Date_start_fr.value+
			"&Date_end_fr="+f.Date_end_fr.value+"&new_Date_end="+new_Date_end_st+"&Comment="+f.Comment.value;
			//+"&="++"&="+id_p="+id_p+"&fl="+fl+"&//+"&inet="+f.inet_pay.value+"&Nic="+f.Nic.value
//		write_temp(s_param);
...
//---------------------------------------------------------------------------------
	function sh_tar(cmd,id_tarifab,name_ab,name_abon,ab_sum,k_tar,perstypes) {
		if (cmd=="del") {
			if (confirm("Удалить тариф "+name_ab+" ("+name_abon+")?")) {alert("в разработке")}
		} else {
			var f = document.forms.ulaForm;
...
//---------------------------------------------------------------------------------
	function sh_con(cmd,id_tar_con,name_cn,name_con,con_sum,opl_period,con_typ,id_tarifab,perstypes) {
		if (cmd=="del") {
			if (confirm("Удалить тариф "+name_cn+" ("+name_con+")?")) {alert("в разработке")}
		} else {
			var f = document.forms.ulaForm;
...
//	document.getElementById("tab_Cust").innerHTML = '';
	document.getElementById("B_Sub").innerHTML = '';
	if(document.getElementById("d_Bill_Dog")){document.getElementById("d_Bill_Dog").innerHTML = '';}//<input name="h_id_Podjezd" type="hidden" />
/*	if (f.Menu_Item.value == "recon") {
			document.getElementById("d_mont").innerHTML = '';
			f.mont[0].selected = true;
	}	*/
...
//		if (f.Menu_Item.value == "con3w") {
		...
		if (document.getElementById("net")) {
//				document.getElementById("abon_pay").innerHTML = '';
			...
			if (f.id_tarifab) {f.id_tarifab.value = "0"}; //f.id_tarifab[0].selected = true; //f.h_id_tarifab.value;
...
//				f.conn[0].selected = true;
		...
		if (document.getElementById("w3")) {
			...
			document.getElementById("addLogin").innerHTML = '';
//	alert("!!");
			//'логин <INPUT name="nic2login" type="button" id="nic2login" onclick="javascript:alert(f.Nic.value);f =document.forms.ulaForm; f.Login.value=f.Nic.value;adjastNet();" value="тот же"><INPUT name="Login" type="text" value="" onChange="adjastNet();" size="12">';
		}
		if (document.getElementById("fio")) {
			f.Fam.value = "";
			f.Name.value = "";
...
<?	$tn_tp = "tn=$TabNum&tp=$TypePers"; //	$GLOBALS['pers']['id_TypePers']
	...
	if ($TypePers != 4) { ?>
			<li><a href='javascript:{ch_param("re_con", "<?	echo $tn_tp; ?>", "Mform"); }'>Подключение</a></li>
...
<?php /*?>	<li><a href='javascript:{op_f("pay","Mform");setTimeout("document.forms.ulaForm.id_town.onchange()",300);}'>Платеж</a></li>	<?php */?>	
			...
			<li><a href='javascript:{ch_param("sh_t_abon","<? echo "$tn_tp&per=0"?>","Mform");}'>Терминал(абон.ошиб)</a></li>		
			<li><a href='javascript:{ch_param("sh_t_inet","<? echo "$tn_tp&per=0"?>","Mform");}'>Терминал(инет.ошиб)</a></li>		
			<li><a href='javascript:{ch_param("sh_v_bad","<? echo "$tn_tp&per=0"?>","Mform");}'>Отключать!</a></li>		
