...
/* if (!$not_new_adr) { //$Nic!=""
	...
	if ($totalRows_customer > 0 and $_REQUEST ["menu"] != "new_adr") {?>
		<button name="B_reload" type=button onClick="clr_adress();chk_adress();">
        					<img src="reload.png" align=middle alt="Обнови"></button>
<?	}
if ($tn==2) { // ### *** ### *** ### ***	строка для отладки	 ### *** ### *** ### *** ?>
	<input onblur="eval(this.value)" size="10"/><? }
...
//			$Bill_Dog = $row_customer['Bill_Dog']; ?>
	<table align="center" border=0 width=100%>
  		<tr>
  		  <?	//	print_r($row_customer);	$s_onchange='"clr_adress(); show_cust('.$Bill_Dog.','.$id_p.','.$fl.'); f_btn();"';
...
/*	<td  rowspan="2"><select name="tabl_cust" id="tabl_cust" class="headText" size="<? echo $totalRows_customer ?>"<? echo ($not_new_adr?' onchange='.$s_onchange:'')?> > <?	*/ ?>
  ...
  <? if ($_REQUEST ["menu"] != "new_adr") { ?>
    <select name="tabl_cust" id="tabl_cust" class="headText" size="<? echo $totalRows_customer; ?>"	onchange="clr_adress(); chk_adress();"<? //echo $s_onchange ?> >
  <? } ?>  
...
<? //!	} else
if (isset($_REQUEST ["Nic"])||$_REQUEST ["menu"]=="show_err"){//<button id="sel_Bill" type="button">уст</button>	?>
    <!--!!!<table><tr>-->
	<td><div id="d_show_err"<?php /*?> style="display:none"<?php */?>><table><tr><td>
...
<?		}	//foreach?></td>
		...
		<!--</tr></table>-->
<?		}	//!?>
<?		if ($_REQUEST ["menu"] != "show_err" and !($_REQUEST ["menu"] == "new_adr" and !isset($_REQUEST ["Nic"])) ) { ?>
    <!--<td><div>-->
    <!--!!!<table><tr>-->
...
<?	$nodis = 'style="display:none"';
	...
	$menu = $_REQUEST ["menu"];?>
    <?php /*?>chk_adress();shw('nNic');if(nNic" <? if (!($m_pay && $a_cus[$Bill_Dog]['inet']==1)) { echo 'style="display:none"'; }?><?php */?>
    <!--</tr><tr>-->
			<td bgcolor="#CCFFFF" id="_rec" align="center" <? echo $menu!="recon"?"":'style="border:solid #CC00FF"'?>>
            	<div id="L_rec" <? echo $menu!="recon"?$nodis:$div_b?>>
                	<b>Сеть</b>
<?                    if ($totalRows_customer > 0) { ?>
                        ...
                        onClick="javascript:B_D=f_Bill_Dog();tn=<? echo $tn?>;if(confirm(&quot;Вы согласны удалить договор№&quot;+B_D+&quot; в архив? &quot;)){
                                    ch_param('do_del_cust','Bill_Dog='+B_D+'&tn='+tn,'tab_Cust');}"><img src="ico_delete.gif" align=middle alt="Удалить"></button>
<?                     }?>
                </div>
            	<div id="B_rec" <? echo $menu=="recon"?$nodis:""?>>
...
<!--			<td id="d_b_rec" bgcolor="#CCFFFF" <? echo 1 or $menu!="recon"?$nodis:""?>>
            </td>
-->			<!--</tr><tr>-->
            <td bgcolor="#FFCC99" id="_pay" align="center" <? echo $menu!="pay"?"":'style="border:solid #FF9999"'?>>
            <div id="L_pay" <? echo $menu!="pay"?$nodis:$div_b?>>
              <table><tr>
           		<td><b>Фин</b></td>
    ...
    <?		if($tp==1 || $tn==6 || $tn==8) { // (!$inet1 && ($tn==2 || $tn==6)) ?>
                ...
                <td><div id="dhd" style="display:none">
                    <table><tr><td><? echo $Bill_Dog1?>:</td>
    <?			if($inet1) {?>
                    <td><div id="toab">
                        <button name="B_2ab" type=button onClick="javascript:document.forms.ulaForm.sBill_Dog.value=<? echo "$Bill_Dog1;ch_param('toab','B=$Bill_Dog1"?>','toab');">Аб.</button></div></td>
    <?         } else { ?>
    ...
    <?		} // if $tp==1 || $tn==6 || $tn==8 ?>
            ...
            <div id="B_pay" <? echo $menu=="pay"?$nodis:""//?>>
	...
	<!--	  <td id="d_b_pay" bgcolor="#FFCC99" <? echo $menu!="pay"?$nodis:""?>><div></div></td>-->
	<!--d_b_pay -->
			<!--</tr><tr>-->
            <td bgcolor="#99FFFF" id="_not" align="center" <? echo $menu!="noti"?"":'style="border:solid #00FFFF"'?> >
            <div id="L_not" <? echo $menu!="noti"?$nodis:$div_b?>><b>Ремонт</b></div>
            <div id="B_not" <? echo $menu=="noti"?$nodis:""?>><button name="B_not" type=button 
	...
	<? //';// valign="baseline" chk_adress(); hid('recon');
...
?>        <div id="d<? echo $id?>" style="display:none">d<? echo $id?></div>
			<input <? echo "name='$inp_name' id='$inp_name' type='hidden' value='$totalRows_login'"?> />
...
?><div id="noti2rep" <? echo $menu=="noti"?"":'style="display:none"';//$nodis?>><?
//	form_phn($phone_Home, $phone_Cell, $phone_Work, $Jur);	
...
<table width="800" border=0 style="background-color:#9FF">
  <tr>
	...
	<td width="725" > 	
	   	<? echo $_cus['phone_Home']==""?"":"домашний - <b>{$_cus['phone_Home']}</b>"; ?>
	   	<? echo $_cus['phone_Cell']==""?"":"сотовый - <b>{$_cus['phone_Cell']}</b>"; ?>
	   	<? echo $_cus['phone_Work']==""?"":" рабочий - <b>{$_cus['phone_Work']}</b>"; ?>
     </td>
  </tr>
...
<?php /*?><table width="800" border=0>
  <tr>
    ...
    <div id="pay" <? echo 'style="display:none"'; //if ($GLOBALS['menu']!='pay') { } ?>>
...
//	echo "st= $Date_start_st, dp=$Date_pay";  ?>
            ...
            <tr>
				<td><? //echo $cust['auto'], ' ', strlen($cust['mac']), ' ', $state, '!';?><label>&nbsp;
                	<input name="ND_<? echo $i?>" type="checkbox" value="<? echo $cust['Bill_Dog']?>"
							onchange="" checked="checked" disabled="disabled" /><? echo $cust['Bill_Dog']?></label></td>
				<td align="center" colspan="1"><? echo $cust['Date_pay']==""?"":sh_date($cust['Date_pay'])?>
...
?><div id="phn" <? if ($GLOBALS['menu']=='pay') { echo 'style="display:none"'; } ?>>
<table width="800" border=0 style="background-color:#9FF">
  <tr>
    <td width="65"  align="left" valign="middle"><strong>телефоны:</strong></td>
...
//require($frm_fio); //============================================================================
?> 
	<div id="fio" <? if ($GLOBALS['menu']!='recon') { echo 'style="display:none"'; } ?>>
    <table width="800" border=0>
  <tr>
...
<!--   -->серия <input name="pasp_Ser" type="text" id="pasp_Ser" onChange="adjastPasp();" value="<? echo $pasp_Ser; ?>" size="3" />
  ...
  <tr>
	...
	<td  align="right">
    <input name="Comment" type="text" value="<? echo $Comment; //id="Comment" ?>" size="97" onchange='adj_Cust();' />
    <? //$tmp=array_keys($a_cus); print_r($a_cus[$tmp[0]]["Comment"]);// //print_arr($a_cus);//[$Bill_Dog]['mac']//echo //?>
      </td>
  </tr>
</table>
</div>
...
//	$m = Array(1=>"января",2=>"февраля",3=>"марта",4=>"апреля",5=>"мая",6=>"июня",7=>"июля",8=>"августа",9=>"сентября",10=>"октября",11=>"ноября",12=>"декабря");
	$m = Array(1=>"янв",2=>"фев",3=>"мар",4=>"апр",5=>"мая",6=>"июн",7=>"июл",8=>"авг",9=>"сен",10=>"окт",11=>"ноя",12=>"дек");
?>
<div id="net" style="background-color:#CCFFFF;" <? if(!$m_rec){?>style="display:none"<? }?>>
    ...
    <div id="net_tab" <?php echo (($m_pay /*&& $a_cus[$Bill_Dog]['inet']==1*/)?'style="display:none"':'');?> >
<table width="800" border=0>
  <tr>
    <td width="20" align="left" <?php //echo ($m_pay?'width="40"':'colspan="3" width="450"');?>><strong>Сеть: </strong></td>
	<td <?php if (!$m_pay || 1) echo 'width="550"'?>align="left">
    	<? //print_r($a_cus[$Bill_Dog]); ?>
	<?php if (!$m_pay || 1) {?>&nbsp;Договор<? }?>
        ...
        ?><input name="mac" id="mac" value="<? echo $mac; ?>" type="text" <? if($m_pay){?>style="display:none"<? }?>
        	size="15" maxlength="17" onkeyup="v_MAC(this.value,<? echo "'{$Bill_Dog}'"?>);" 
            onchange='<? echo /*$GLOBALS['tp']>2?*/"f=document.forms.ulaForm.tabl_cust;cor_mac(f.options[f.selectedIndex].value)"/*:"adj_mac()"*/;?>'/>
		<?php if (!$m_pay || 1) {?>&nbsp;ник<? }?>
		<input name="Nic" type="text" <?php echo ($m_pay && 0?'style="display:none"':'');?> id="Nic" <? if($GLOBALS['tp']<3) {?>onChange="adj_Nic(this);"<? }?> value="<? echo $Nic; ?>" size="9" />
      	&nbsp;<?php echo ($m_pay && 0?'':'&nbsp;подключ.'); // ?>
	<select name='conn' id='conn' <? if($GLOBALS['tp']<3) {?>onchange='adj_Conn(this.value);'<? }?> class='headText' <?php echo $m_pay && 0?'style="display:none"':'';?> >
<? 	if ($conn>0) {
...
/*echo $conn," ",*/ $s_qer = "SELECT * FROM `v_tarifab` WHERE `con_typ`=".$c_typ.(($c_typ==4)?" or (`id_tar_con`=1 and `con_sum`<500)":"")." order by `id_tar_con`";
			...
			$nm_tarif = "";	?> 
		<select name='id_tar_con' class='font8pt' id='id_tar_con' <? echo ($GLOBALS['tp']<3?"onchange='adj_con_tar(this)'":"").$m_pay && 0?"style='display:none'":"" ?> >	
...
<?		foreach ($tarifs as $t_row) {		//	print_r($t_row);
    ...
    <td align="left" <?php echo ($m_pay && 0?'style="display:none"':'');//<div id="conn_pay" ></div>?>>
...
<table width="800" border=0>
  <tr><td width="10"></td>
<? }?>
	<td <? if(!$m_pay || 1){?><? }?>width="220"><div id="con_s">Абон.тариф<b>
		<? if ($id_tarifab>0) {
				if ($id_tarifab==3) {?>
...
<?php /*	?>		<select name="id_tarifab" class='font8pt' id="id_tarifab" onchange='adj_con_tar();' <?php echo ($m_pay?'style="display:none"':'');?>>
	...
	<td <? if(!$m_pay || 1){?>width="0"<? }?> align="left"><div id="abon_pay">
...
//!		}
	...
	<? } else {//?>
		<td width="300" align="right"><div id="state" style="border:solid #<? echo $ar_c[$state] ?>">&nbsp;<strong><? echo $ar_s[$state]; ?></strong>&nbsp;
            ...
            по <input name="Date_end_st" value="<? echo $Date_end_st; ?>" size="8" onchange="document.forms.ulaForm.Date_pay.value=this.value;adj_CPay_act()" type="date"/></div></td>
	<? }?>
	<td align="left"><div id="Date_pay"> оплачено по <input name="Date_pay" value="<? echo $Date_pay; ?>" size="8" <? if($m_pay && 0){?>type="hidden"<? }?>/>
		<? if($m_pay && 0){ echo '<b>'.(empty($Date_pay)?'___':date("j ",strtotime($Date_pay)).$m[date("n",strtotime($Date_pay))].' '.date("Y", strtotime($Date_pay)).'г.').'</b>';
		//$Date_pay; 
		}
...
<? if(!$m_pay || 1){ echo $is_frend=isset($a_cus[$Bill_Dog]['Bill_frend'])/*&&($frend=$a_cus[$Bill_Dog]['Bill_frend'])!=""*/?"друг: ":""; ?>
...
<? #######################################################################################################?>
<div id="p_net" <? if($GLOBALS['menu'] != 'pay') echo 'style="display:none"'?> >
<?php /*?><div id="nNic" <? if (!(/*$m_pay && * /$a_cus[$Bill_Dog]['inet']==1)) { echo 'style="display:none"'; }?>>
        <table bgcolor="#FF9900" width="800"><tr align="center" height="40">
            <td>Установить <input name="B_err_nic" type="button" onclick='ch_param("err_nic","","nNic");' value="соответствие сетевому нику из Ошибок базы" /></td>
            <td>или как доп.инет.учётка к абон.договору №&nbsp;
                <input name="NewBill" id="NewBill" value="<? echo $Bill_Dog; ?>" type="text" size="4" 
...
<table id="p_net_tab" width="800" border=0 style="background-color:#CCFFFF" <?php echo (($m_pay && $a_cus[$Bill_Dog]['inet']==1)?'style="display:none"':'');?>>
  <tr>
    <td align="left">
      	<strong>Сеть: </strong>&nbsp;<?php echo ($m_pay?'':'&nbsp;подключ.'); // ?>
		<?php echo ($m_pay?'<b>&nbsp;'.$nm_tarif.'&nbsp;</b>':''); ?>
		Абон.тариф<b>
		<? if ($id_tarifab>0) {
				if ($id_tarifab==3) {?>
	...
	<? // $ar_c = array(''=>'333333', '0'=>'333333', '1'=>'33CC66', '2'=>'0000FF', '3'=>'00FFFF'); ?>
    <td align="center"><div id="p_state" style="border:solid #<? echo $ar_c[$state] ?>">&nbsp;<b><? echo $ar_s[$state]; ?></b>&nbsp;
    c <? echo $Date_start_st; ?>&nbsp;
    <? if ($state==2 && $Date_end_st=='') { //?> за долг<? } else { ?>
        по <? echo $Date_end_st; } ?>&nbsp;
    </div></td>
	<td align="left"><div id="p_Date_pay"> оплачено по 
		<?  echo '<b>'.(empty($Date_pay)?'___':date("j ", strtotime($Date_pay)).$m[date("n", strtotime($Date_pay))].' '.date("Y", strtotime($Date_pay)).'г.').'</b>';
		if ($GLOBALS['totalRows_customer']>0 && $a_cus[$Bill_Dog]['dolg'] && $Date_end_st!='' && $m_ab>0 && $a_cus[$Bill_Dog]['inet']==0) { ?><font size="3" style="background-color:#FF0000" color="#FFFF00"><br><b>Нет заявки на откл!</b></font>
   <? if ($m_pay)	put_noti2off ($Date_pay, $Bill_Dog, $a_cus[$Bill_Dog]['Cod_flat'], $a_cus[$Bill_Dog]['id_Podjezd'], $a_cus[$Bill_Dog]['flat'], $GLOBALS['tn']/*TabNum*/);
		}?>
        <? /**/echo isset($a_cus[$Bill_Dog]['rad']) && $a_cus[$Bill_Dog]['rad']?"<b>R√":"R-";/**/?>
    	</div>    
    </td>
  </tr>
</table>
...
<div id="w3" border=1 style="background-color:#CCFFCC">
 <table id="inet_tab" width="800" border="0" <?php echo (($m_pay && $GLOBALS['inet1']==1)?'style="display:none"':'');?>>
  <tr>
    <td width="106" align="<?php if ($m_pay && 0) {?>right<? }else{?>left<? }?>"><strong>Интернет&nbsp;
...
<?php /*?>	<td width="35" align="right" <?php echo (($GLOBALS['menu']=='pay')?'':'width="40"');?>>логин </td><?php */?>
	<td width="220"><div id="Login" align="left<?php //if ($m_pay) {? ><? }else{? >rihgt<? }?>">
<?php 	if (($GLOBALS['totalRows_customer']>0) && ($GLOBALS['Logins'][$GLOBALS['Bill_Dog1']]["Logins"]>0)) {
			...
			<select name="Login" id="Login" size="<? echo ''.$inp_sz.'';?>" onchange="adjustLogin()" class="navText" >
<?			for($i=1; $i<=$inp_sz; $i++){
...
/*			$Log1 = $GLOBALS['Logins'][$GLOBALS['Bill_Dog1']][1];
	...
	/*		echo //'<td valign="bottom">'.
				'<input name="addLogin" type="button" id="addLogin" onclick="faddLogin();" value="+"'.
						(($GLOBALS['menu']=='pay')?'style="display:none"':'').'/>'.
				'</td>';//</tr>	*/
		} elseif ($m_pay) {	//echo '<b>логин отсутствует!</b>';	?>
...
<?		} else {
	...
	<td width="540"><div id="nw3"<?php if ($GLOBALS['menu']!='recon') { echo 'style="display:none"'; }?>>
	<table><tr>
    	<td id="daddLogin"><?php if ($GLOBALS['totalRows_customer']>0){?><input name="addLogin" type="button" onclick="faddLogin();" value="+"/><? }?></td><!-- id="addLogin"-->
	<?php /*?>	<td align="left" <?php echo $m_pay && 0?'style="display:none"':'' ?>><div id="addLogin"></div></td><?php */?>
		<td align="center" id="tarif3w"><?php if ($GLOBALS['menu']!='pay' || 1) {?>тариф<? }?>
    	<select name="id_tarif3w" class='headText' id="id_tarif3w" <? echo $GLOBALS['tp']<3?"onchange='adjustTarif3w()'":""?> <?php echo $m_pay && 0?'style="display:none"':'' ?>>
      <?php
	...
	<td align="left" id="d_t3w_date" <?php echo (($GLOBALS['menu']=='pay' && 0)?'style="display:none"':'');?>><div>установлен с 
...
<!--    <input name="dt3w2day" type="button" id="dt3w2day" onclick="javascript:document.forms['ulaForm'].tarif3w_date.value=TODAY2" value="сегодня" />--></div></td>
    ...
    <td><?php if (!$m_pay || 1) {?>из сети <? }?>
   	  <input name="From_Net" id="From_Net" type="text" <?php echo $m_pay && 0?'style="display:none"':'' ?> <? echo $GLOBALS['tp']<3?'onChange="adjastNet()"':''?> value="<? echo $From_Net?>" size="7" />
    </td>
