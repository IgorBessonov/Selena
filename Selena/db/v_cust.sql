...
`customers`.`floor` AS `floor` 
from (((((((`customers` join `spr_street`)
	 left join `spr_tarifab` on((`customers`.`id_tarifab` = `spr_tarifab`.`id_tarifab`)))
	 left join `spr_podjezd` on((`customers`.`id_Podjezd` = `spr_podjezd`.`id_Podjezd`)))
	 join `spr_build` on(((`spr_podjezd`.`id_korp` = `spr_build`.`id_korp`) and (`spr_build`.`id_street` = `spr_street`.`id_street`))))
	 left join `spr_region` on((`spr_build`.`id_Region` = `spr_region`.`id_Region`)))
	 left join `personal` on((`spr_region`.`id_Region` = `personal`.`id_Region`)))
	 join `spr_town` on((`spr_street`.`id_Town` = `spr_town`.`id_Town`)));