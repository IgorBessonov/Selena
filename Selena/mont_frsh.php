...
$q_DP = "$q_ and `Date_Fact` IS NULL and DATE_FORMAT(`Date_Plan`,'%Y-%m-%d')=>'$Date_Plan' and TN_canc is null";
$q_DP = "$q_ and `Date_Fact` IS NULL and DATE_FORMAT(`Date_Plan`,'%Y-%m-%d')<=ADDDATE(CURDATE(), 3) 
							 and DATE_FORMAT(`Date_Plan`,'%Y-%m-%d')>=SUBDATE(CURDATE(), 3) and TN_canc is null and auto = 0";
if (!isset($_REQUEST ["s"])) {
 ?><input name="s" type="checkbox" value="<? echo $s?>" <? if($s==1) {?>checked="checked"<? }?> onchange='this.value=this.value==1?0:1;ch_param("<? echo $prog."\",\"". $param?>s="+this.value,"dmont")' />
...
if($tp<=4) {
	...
	$result = mysql_query($q_noti) or die($q_noti." ".mysql_error());?>
	<table border="0" cellpadding="2" cellspacing="1" >
		<tr>
	<? if (!isset($_REQUEST ["m_TabNum"])) { ?>
			<td align="center" colspan="2"><h3>
				<? if (mysql_num_rows($result) == 0/*!isset($result)*/) { ?>
        ...
        <tr><td>Регион (заявок)</td></tr>
			<? 	while ($row = mysql_fetch_assoc($result))  {	?>
                <tr>
                    <td bgcolor="#E5E5E5"><b>
					<a href='javascript:{ch_param("<? echo $prog."\",\"". $param?>m_TabNum=<? echo $row["m_TabNum"]?>","d_TN<? echo $row["m_TabNum"]?>");}'> <? echo $row["RegionName"]?> </a>&nbsp;</b>(<? echo $row["coun"];?>)
...
if (!isset($_REQUEST ["name_street"])) { ?>
	<table border="0">
		<? 	while ($row = mysql_fetch_assoc($result))  {	?>
            <tr><td width="10"></td>
                <td bgcolor="#E5E5E5"><b>
                <a href='javascript:{ch_param("<? echo $prog."\",\"". $param."m_TabNum=$m_TabNum&name_street=".$row["name_street"]?>","d_s<? echo $row["name_street"]?>");}'> <? echo $row["name_street"]?> </a>&nbsp;</b>(<? echo $row["coun"];?>)<div style="border:thin #009 solid" id="d_s<? echo $row["name_street"]?>"></div></td>
            </tr>
        <?    }	?>
    </table>
	<?
...
if (!isset($_REQUEST ["Num_build"])) { ?>
	...
	<!--	<tr><td>Дом</td><td>заявок</td></tr>-->
		...
		<? 	while ($row = mysql_fetch_assoc($result))  {	?>
            <tr><td width="10"></td>
                <td bgcolor="#E5E5E5"><b>
                <a href='javascript:{ch_param("<? echo $prog."\",\"". $param?>m_TabNum=<? echo $m_TabNum?>&name_street=<? echo $name_street?>&Num_build=<? echo $row["Num_build"]?>","d_b<? echo $name_street.$row["Num_build"]?>");}'> <? echo $row["Num_build"]?> </a>&nbsp;</b>(<? echo $row["coun"];?>)<div id="d_b<? echo $name_street.$row["Num_build"]?>"></div></td>
        <?    }	?>
            </tr>
...
$q_noti = "SELECT RegionName,Num_Notify,Bill_Dog,Date_in,Korpus,Podjezd,`floor`,flat,Fam,Name,Father,phone_Cell,phone_Home,phone_Work,phone_Dop,Notify,Date_Plan,`comment`,Date_Fact, conn FROM v_notify WHERE Date_Fact IS NULL AND $q_DP AND $q_TN and $q_str AND $q_bld order by space(5-length(flat))+flat";//
...
<?php /*?>	<tr>
	...
	<td align="center" ><strong><!--Улица дом.-->кв.<br />
															 (подъезд, эт.)</strong></td>
...
<?php */?>
<?		$bgc = ($i++ % 3) ? $cfg['BgcolorOne'] : $cfg['BgcolorTwo'];		$NNoti = $row["Num_Notify"] ?>
		<td align="center" bgcolor="<?php echo $bgc; ?>"> <?php echo $NNoti;						?> </td>
		...
		<td bgcolor="<?php echo $bgc; ?>"> <?php echo $row["Fam"].'<br>'.$row["Name"].' '.$row["Father"]; 			?> </td>
		...
		<td bgcolor="<?php echo $bgc; ?>"> <?php echo $row["Notify"]; ?>
		...
		<td bgcolor="<?php echo $bgc; ?>"><input name="d_fact<?php echo $NNoti?>" type="date" size="8"
        	onchange='ch_param("do_cls_noti","com="+document.getElementById("c<? echo $NNoti?>").value+"<? echo "&conn=".$row["conn"]."&Notify=$NNoti&tn=$tn&Bill_Dog=".$row["Bill_Dog"]."&Date_Fact="?>"+this.value,"d<? echo $NNoti?>"); this.disabled="disabled";document.getElementById("c<? echo $NNoti?>").disabled="disabled";' /> </td>
